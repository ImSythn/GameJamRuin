...
#include "Ruin/Character/RuinCharacterMovementComponent.h"
...
#include "Ruin/Sound/RuinMoveStepSound.h"
#include "Ruin/Character/RuinCharacter.h"
...
void URuinCharacterMovementComponent::InitializeComponent()
{
	Super::InitializeComponent();
	RuinCharacter = Cast<ARuinCharacter>(GetOwner());
}

void URuinCharacterMovementComponent::OnRegister()
{
	Super::OnRegister();
...
void URuinCharacterMovementComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{	
	...
	{
		FRotator ControlRotation = RuinCharacter->GetController()->GetControlRotation();
		ControlRotation.Roll = GetCameraRoll();
		RuinCharacter->GetController()->SetControlRotation(ControlRotation);
	}
...
bool URuinCharacterMovementComponent::DoJump(bool bClientSimulation)
{
...
void URuinCharacterMovementComponent::TwoWallAdjust(FVector& Delta, const FHitResult& Hit, const FVector& OldHitNormal) const
{
	Super::TwoWallAdjust(Delta, Hit, OldHitNormal);
}

float URuinCharacterMovementComponent::SlideAlongSurface(const FVector& Delta, float Time, const FVector& Normal, FHitResult& Hit, bool bHandleImpact)
{
	return Super::SlideAlongSurface(Delta, Time, Normal, Hit, bHandleImpact);
}

FVector URuinCharacterMovementComponent::ComputeSlideVector(const FVector& Delta, const float Time, const FVector& Normal, const FHitResult& Hit) const
{
	return Super::ComputeSlideVector(Delta, Time, Normal, Hit);
}

FVector URuinCharacterMovementComponent::HandleSlopeBoosting(const FVector& SlideResult, const FVector& Delta, const float Time, const FVector& Normal, const FHitResult& Hit) const
{
...
bool URuinCharacterMovementComponent::ShouldCatchAir(const FFindFloorResult& OldFloor, const FFindFloorResult& NewFloor)
{
...
bool URuinCharacterMovementComponent::IsWithinEdgeTolerance(const FVector& CapsuleLocation, const FVector& TestImpactPoint, const float CapsuleRadius) const
{
	return Super::IsWithinEdgeTolerance(CapsuleLocation, TestImpactPoint, CapsuleRadius);
}

bool URuinCharacterMovementComponent::ShouldCheckForValidLandingSpot(float DeltaTime, const FVector& Delta, const FHitResult& Hit) const
{
...
bool URuinCharacterMovementComponent::IsValidLandingSpot(const FVector& CapsuleLocation, const FHitResult& Hit) const
{
	if (!Hit.bBlockingHit)
	{
		return false;
...
void URuinCharacterMovementComponent::TraceCharacterFloor(FHitResult& OutHit)
{
...
void URuinCharacterMovementComponent::OnMovementModeChanged(EMovementMode PreviousMovementMode, uint8 PreviousCustomMode)
{
...
float URuinCharacterMovementComponent::GetCameraRoll()
{
...
void URuinCharacterMovementComponent::SetNoClip(bool bNoClip)
{
...
void URuinCharacterMovementComponent::ToggleNoClip()
{
	SetNoClip(!bCheatFlying);
}

void URuinCharacterMovementComponent::ApplyVelocityBraking(float DeltaTime, float Friction, float BrakingDeceleration)
{
...
bool URuinCharacterMovementComponent::ShouldLimitAirControl(float DeltaTime, const FVector& FallAcceleration) const
{
	return false;
}

FVector URuinCharacterMovementComponent::NewFallVelocity(const FVector& InitialVelocity, const FVector& Gravity, float DeltaTime) const
{
...
void URuinCharacterMovementComponent::UpdateCharacterStateBeforeMovement(float DeltaSeconds)
{
...
void URuinCharacterMovementComponent::UpdateCharacterStateAfterMovement(float DeltaSeconds)
{
...
void URuinCharacterMovementComponent::UpdateSurfaceFriction(bool bIsSliding)
{
...
void URuinCharacterMovementComponent::UpdateCrouching(float DeltaTime, bool bOnlyUncrouch)
{
...
URuinMoveStepSound* URuinCharacterMovementComponent::GetMoveStepSoundBySurface(EPhysicalSurface SurfaceType) const
{
	TSubclassOf<URuinMoveStepSound>* GotSound = RuinCharacter->GetMoveStepSound(TEnumAsByte<EPhysicalSurface>(SurfaceType));
...
void URuinCharacterMovementComponent::PlayMoveSound(const float DeltaTime)
{
	...
	URuinMoveStepSound* MoveSound = nullptr;
...
void URuinCharacterMovementComponent::PlayJumpSound(const FHitResult& Hit, bool bJumped)
{
	if (!bShouldPlayMoveSounds)
	{
		return;
	...
	URuinMoveStepSound* MoveSound = nullptr;
	TSubclassOf<URuinMoveStepSound>* GotSound = nullptr;
	if (Hit.PhysMaterial.IsValid())
	{
		GotSound = RuinCharacter->GetMoveStepSound(Hit.PhysMaterial->SurfaceType);
	}
	if (GotSound)
	{
	...
	if (!MoveSound)
	{
		if (!RuinCharacter->GetMoveStepSound(TEnumAsByte<EPhysicalSurface>(SurfaceType_Default)))
		{
			return;
		}
		MoveSound = RuinCharacter->GetMoveStepSound(TEnumAsByte<EPhysicalSurface>(SurfaceType_Default))->GetDefaultObject();
	}
	...
	if (MoveSound)
	{
		...
		if (!bJumped)
		{
			...
			if (FallSpeed > RuinCharacter->GetMinSpeedForFallDamage())
			{
				MoveSoundVolume = 1.0f;
			}
			else if (FallSpeed > RuinCharacter->GetMinSpeedForFallDamage() / 2.0f)
			{
				MoveSoundVolume = 0.85f;
			}
			else if (FallSpeed < RuinCharacter->GetMinLandBounceSpeed())
			{
		...
		{
			MoveSoundVolume = RuinCharacter->IsSprinting() ? MoveSound->GetSprintVolume() : MoveSound->GetWalkVolume();
		}
...
void URuinCharacterMovementComponent::CalcVelocity(float DeltaTime, float Friction, bool bFluid, float BrakingDeceleration)
{
	...
	if (bCheatFlying)
	{
		...
		else
		{
			...
			auto NoClipAccelClamp = RuinCharacter->IsSprinting() ? 2.0f * MaxAcceleration : MaxAcceleration;
...
void URuinCharacterMovementComponent::Crouch(bool bClientSimulation)
{
...
void URuinCharacterMovementComponent::DoCrouchResize(float TargetTime, float DeltaTime, bool bClientSimulation)
{
...
void URuinCharacterMovementComponent::UnCrouch(bool bClientSimulation)
{
	// TODO: replicate to the client simulation that we are in a crouch transition so they can do the resize too.
	if (bClientSimulation)
	{
...
void URuinCharacterMovementComponent::DoUnCrouchResize(float TargetTime, float DeltaTime, bool bClientSimulation)
{
...
bool URuinCharacterMovementComponent::CanAttemptJump() const
{
...
float URuinCharacterMovementComponent::GetMaxSpeed() const
{
	if (bCheatFlying)
	{
		return (RuinCharacter->IsSprinting() ? SprintSpeed : WalkSpeed) * 1.5f;
	}
	float Speed;
	if (RuinCharacter->IsSprinting())
	{
	...
	else if (RuinCharacter->DoesWantToWalk())
	{
		Speed = WalkSpeed;
	}
