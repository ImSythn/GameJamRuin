	...
	: Super(ObjectInitializer.SetDefaultSubobjectClass<URuinCharacterMovementComponent>(ACharacter::CharacterMovementComponentName))
{
	...
	MovementPtr = Cast<URuinCharacterMovementComponent>(ACharacter::GetMovementComponent());
...
void ARuinCharacter::ApplyDamageMomentum(float DamageTaken, FDamageEvent const& DamageEvent, APawn* PawnInstigator, AActor* DamageCauser)
{
...
void ARuinCharacter::ClearJumpInput(float DeltaTime)
{
...
void ARuinCharacter::OnMovementModeChanged(EMovementMode PrevMovementMode, uint8 PrevCustomMode)
{
	if (!bPressedJump)
	{
		ResetJumpState();
...
void ARuinCharacter::StopJumping()
{
	if (!bDeferJumpStop)
	{
		Super::StopJumping();
...
void ARuinCharacter::OnJumped_Implementation()
{
...
void ARuinCharacter::ToggleNoClip()
{
	MovementPtr->ToggleNoClip();
}

bool ARuinCharacter::CanJumpInternal_Implementation() const
{
...
void ARuinCharacter::Move(FVector Direction, float Value)
{
...
void ARuinCharacter::Turn(bool bIsPure, float Rate)
{
	if (!bIsPure)
	{
...
void ARuinCharacter::LookUp(bool bIsPure, float Rate)
{
	if (!bIsPure)
	{
...
void ARuinCharacter::RecalculateBaseEyeHeight()
{
...
bool ARuinCharacter::CanCrouch() const
{
